...
function handleSwarmTool(name, args) {
  switch (name) {
    ...
    case "swarm_get_messages":
      const messages = coordinator.getMessages(args.agent_id, args.since);
      return JSON.stringify(messages);
    case "swarm_create_proposal":
      const proposal = coordinator.createProposal(args.proposer, args.topic, args.options, args.duration_ms || 60000);
      return JSON.stringify(proposal);
    case "swarm_vote":
      const voteResult = coordinator.vote(args.proposal_id, args.agent_id, args.option_index);
      return JSON.stringify({ success: voteResult });
    case "swarm_set_memory":
      const mem = coordinator.setSharedMemory(args.key, args.value, args.updated_by);
      return JSON.stringify(mem);
    case "swarm_get_memory":
      const entry = coordinator.getSharedMemory(args.key);
