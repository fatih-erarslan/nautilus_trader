//! Hardware detection and optimization for IQAD
...
fn detect_amd_gpu() -> bool {
    // Check for AMD GPU via system info
...
pub fn get_optimal_device() -> String {
    ...
    if caps.has_cuda {
        info!("Using CUDA GPU for quantum simulation");
        "cuda".to_string()
    } else if caps.has_apple_gpu {
        info!("Using Apple Silicon GPU for quantum simulation");
        "metal".to_string()
    } else if caps.has_opencl {
        info!("Using OpenCL for quantum simulation");
        "opencl".to_string()
    } else if caps.has_webgpu {
        info!("Using WebGPU for quantum simulation");
        "webgpu".to_string()
    } else if caps.has_avx512 {
        info!("Using AVX-512 CPU for quantum simulation");
        "cpu-avx512".to_string()
    } else if caps.has_avx2 {
        info!("Using AVX2 CPU for quantum simulation");
        "cpu-avx2".to_string()
    } else {
        info!("Using standard CPU for quantum simulation");
        "cpu".to_string()
    }
}
...
pub mod simd {
    ...
    /// Select optimal SIMD implementation (fallback)
