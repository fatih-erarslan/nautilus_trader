...
impl PyAntifragilityMeasurer {
    ...
    fn measure_antifragility(&mut self, returns: &PyArray1<f64>) -> PyResult<PyAntifragilityMeasurement> {
        let returns_slice = returns.as_slice()
            .map_err(|_| PyErr::new::<pyo3::exceptions::PyValueError, _>("Invalid returns array"))?;
...
impl PyBarbellStrategy {
    ...
    fn calculate_position_sizes([^)]*, _market_data: &PyMarketData) -> PyResult<HashMap<String, f64>> {
        self.inner.calculate_position_sizes(&assets, &market_data.inner)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))
    }
    
    fn update_strategy([^)]*, _market_data: &PyMarketData) -> PyResult<()> {
        self.inner.update_strategy(&market_data.inner)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))
    }
    
    fn expected_return([^)]*, _market_data: &PyMarketData) -> PyResult<f64> {
        self.inner.expected_return(&market_data.inner)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))
    }
    
    fn risk_metrics([^)]*, _market_data: &PyMarketData) -> PyResult<PyStrategyRiskMetrics> {
    ...
    fn is_suitable([^)]*, _market_data: &PyMarketData) -> PyResult<bool> {
        self.inner.is_suitable(&market_data.inner)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))
    }
    
    fn calculate_capacity([^)]*, _market_data: &PyMarketData) -> PyResult<f64> {
