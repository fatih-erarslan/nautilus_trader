...
use crate::quantum_core::{
    DeviceType, QuantumCircuit, QuantumDevice, QuantumError, QuantumGate, QuantumResult,
    QuantumState, F64Ext,
};
// Nalgebra imports removed to eliminate warnings
use num_complex::Complex64;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::sync::{Arc, Mutex};
// Tracing imports removed to eliminate warnings
...
impl QuantumTalebianRisk {
    ...
    pub fn new(config: QuantumTalebianConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let quantum_device = Some(QuantumDevice::new_simple(config.device_type));
    ...
    pub fn quantum_black_swan_detection(
        &self,
        market_data: &[f64],
    ) -> QuantumResult<Vec<BlackSwanEvent>> {
        let quantum_device = self
            .quantum_device
            .as_ref()
            .ok_or_else(|| QuantumError::device_error("Quantum device not available"))?;

        // Create quantum state encoding market data for anomaly detection
        let mut _quantum_state = self.encodemarket_data_for_anomaly_detection(market_data)?;

        // Apply quantum black swan detection circuit
        let black_swan_circuit = self.get_or_create_circuit("black_swan_detection", || {
            self.create_black_swan_detection_circuit(market_data.len())
        })?;

        let quantum_state = black_swan_circuit.execute(quantum_device)?;

        // Extract black swan events
        let black_swan_events =
            self.extract_black_swan_events_from_state(&quantum_state, market_data)?;
    ...
    pub fn quantum_tail_risk_assessment(&self, returns: &[f64]) -> QuantumResult<TailRiskMetrics> {
        ...
        let mut _quantum_state = self.encode_return_distribution(returns)?;

        // Apply quantum tail risk assessment circuit
        let tail_risk_circuit = self.get_or_create_circuit("tail_risk_assessment", || {
            self.create_tail_risk_assessment_circuit(returns.len())
        })?;

        let quantum_state = tail_risk_circuit.execute(quantum_device)?;

        // Extract tail risk metrics
        let tail_risk_metrics =
            self.extract_tail_risk_metrics_from_state(&quantum_state, returns)?;

        Ok(tail_risk_metrics)
    }

    /// Quantum antifragility measurement
    pub fn quantum_antifragility_measurement(
        &self,
        stress_data: &[f64],
        performance_data: &[f64],
    ) -> QuantumResult<AntifragilityMetrics> {
        let quantum_device = self
            .quantum_device
            .as_ref()
            .ok_or_else(|| QuantumError::device_error("Quantum device not available"))?;
        ...
        let mut _quantum_state =
            self.encode_stress_performance_relationship(stress_data, performance_data)?;

        // Apply quantum antifragility measurement circuit
        let antifragility_circuit = self
            .get_or_create_circuit("antifragility_measurement", || {
                self.create_antifragility_measurement_circuit(stress_data.len())
            })?;

        let quantum_state = antifragility_circuit.execute(quantum_device)?;

        // Extract antifragility metrics
        let antifragility_metrics = self.extract_antifragility_metrics_from_state(
            &quantum_state,
            stress_data,
            performance_data,
        )?;
    ...
    pub fn quantum_convexity_optimization(&self, portfolio_data: &[f64]) -> QuantumResult<f64> {
        ...
        let mut _quantum_state = self.encode_portfolio_convexity(portfolio_data)?;

        // Apply quantum convexity optimization circuit
        let convexity_circuit = self.get_or_create_circuit("convexity_optimization", || {
            self.create_convexity_optimization_circuit(portfolio_data.len())
        })?;

        let quantum_state = convexity_circuit.execute(quantum_device)?;
    ...
    pub fn quantum_barbell_strategy(
        &self,
        safe_assets: &[f64],
        risky_assets: &[f64],
    ) -> QuantumResult<(f64, f64)> {
        let quantum_device = self
            .quantum_device
            .as_ref()
            .ok_or_else(|| QuantumError::device_error("Quantum device not available"))?;
        ...
        let mut _quantum_state = self.encode_barbell_allocation(safe_assets, risky_assets)?;

        // Apply quantum barbell optimization circuit
        let barbell_circuit = self.get_or_create_circuit("barbell_strategy", || {
            self.create_barbell_strategy_circuit(safe_assets.len() + risky_assets.len())
        })?;

        let quantum_state = barbell_circuit.execute(quantum_device)?;

        // Extract optimal barbell allocation
        let (safe_allocation, risky_allocation) =
            self.extract_barbell_allocation_from_state(&quantum_state)?;

        Ok((safe_allocation, risky_allocation))
    }

    /// Quantum option-like payoff optimization
    pub fn quantum_option_payoff_optimization(
        &self,
        strike_prices: &[f64],
        market_data: &[f64],
    ) -> QuantumResult<HashMap<String, f64>> {
        let quantum_device = self
            .quantum_device
            .as_ref()
            .ok_or_else(|| QuantumError::device_error("Quantum device not available"))?;
        ...
        let mut _quantum_state = self.encode_option_payoffs(strike_prices, market_data)?;

        // Apply quantum option payoff optimization circuit
        let option_circuit = self.get_or_create_circuit("option_payoff_optimization", || {
            self.create_option_payoff_optimization_circuit(strike_prices.len())
        })?;

        let quantum_state = option_circuit.execute(quantum_device)?;

        // Extract optimal option strategy
        let option_strategy =
            self.extract_option_strategy_from_state(&quantum_state, strike_prices)?;
    ...
    pub fn generate_risk_report(
        &mut self,
        market_data: &[f64],
    ) -> QuantumResult<TalebianRiskReport> {
        ...
        let antifragility_metrics =
            self.quantum_antifragility_measurement(&stress_data, market_data)?;

        // Calculate overall scores
        let overall_risk_score =
            self.calculate_overall_risk_score(&tail_risk_metrics, &black_swan_events)?;
        ...
        let confidence_level =
            self.calculate_confidence_level(&tail_risk_metrics, &antifragility_metrics)?;
    ...
    fn encodemarket_data_for_anomaly_detection(
        &self,
        market_data: &[f64],
        ...
        for (i, &data_point) in market_data.iter().enumerate() {
            if i < num_qubits {
                ...
                quantum_state.set_complex_amplitude(i, amplitude)?;
            }
        }
    ...
    fn encode_stress_performance_relationship(
        &self,
        stress_data: &[f64],
        performance_data: &[f64],
    ) -> QuantumResult<QuantumState> {
    ...
    fn create_black_swan_detection_circuit(
        &self,
        _data_length: usize,
        ...
        for i in 0..num_qubits {
            circuit.add_gate(
                QuantumGate::rotation_z(i, std::f64::consts::PI / 8.0)?,
                vec![i],
            )?;
        }
    ...
    fn create_tail_risk_assessment_circuit(
        &self,
        _data_length: usize,
    ) -> QuantumResult<QuantumCircuit> {
        let num_qubits = self.config.num_qubits;
        ...
        for i in 0..num_qubits {
            circuit.add_gate(
                QuantumGate::rotation_x(i, std::f64::consts::PI / 4.0)?,
                vec![i],
            )?;
        }
        ...
        for i in 0..num_qubits - 1 {
            circuit.add_gate(
                QuantumGate::controlled_phase(i, i + 1, std::f64::consts::PI / 6.0)?,
                vec![i, i + 1],
            )?;
        }
        ...
        for i in 0..num_qubits {
            ...
            circuit.add_gate(QuantumGate::RZ { qubit: i, angle }, vec![i])?;
        }
    ...
    fn create_antifragility_measurement_circuit(
        &self,
        _data_length: usize,
    ) -> QuantumResult<QuantumCircuit> {
        let num_qubits = self.config.num_qubits;
        ...
        for i in 0..num_qubits {
            circuit.add_gate(
                QuantumGate::rotation_x(i, std::f64::consts::PI / 6.0)?,
                vec![i],
            )?;
        }
    ...
    fn create_convexity_optimization_circuit(
        &self,
        _data_length: usize,
    ) -> QuantumResult<QuantumCircuit> {
        let num_qubits = self.config.num_qubits;
        ...
        for i in 0..num_qubits - 1 {
            circuit.add_gate(
                QuantumGate::controlled_phase(i, i + 1, std::f64::consts::PI / 4.0)?,
                vec![i, i + 1],
            )?;
        }
    ...
    fn create_barbell_strategy_circuit(&self, _data_length: usize) -> QuantumResult<QuantumCircuit> {
        let num_qubits = self.config.num_qubits;
        let mut circuit = QuantumCircuit::new(num_qubits);
        ...
        for i in 0..num_qubits {
            circuit.add_gate(
                QuantumGate::rotation_x(i, std::f64::consts::PI / 8.0)?,
                vec![i],
            )?;
        }
    ...
    fn create_option_payoff_optimization_circuit(
        &self,
        _numstrikes: usize,
    ) -> QuantumResult<QuantumCircuit> {
        let num_qubits = self.config.num_qubits;
        ...
        for i in 0..num_qubits {
            circuit.add_gate(
                QuantumGate::rotation_x(i, std::f64::consts::PI / 12.0)?,
                vec![i],
            )?;
        }
    ...
    fn extract_black_swan_events_from_state(
        &self,
        ...
        for (_i, amplitude) in amplitudes.iter().enumerate() {
            ...
            if anomaly_score > self.config.black_swan_threshold {
                let event = BlackSwanEvent {
                    ...
                    antifragility_opportunity: self
                        .calculate_antifragility_opportunity(anomaly_score),
                };
                events.push(event);
            }
        }
    ...
    fn extract_tail_risk_metrics_from_state(
        &self,
        quantum_state: &QuantumState,
        returns: &[f64],
    ) -> QuantumResult<TailRiskMetrics> {
        let amplitudes = &quantum_state.complex_amplitudes;
    ...
    fn extract_antifragility_metrics_from_state(
        &self,
        quantum_state: &QuantumState,
        stress_data: &[f64],
        performance_data: &[f64],
        ...
        let antifragility_coefficient =
            amplitudes.iter().map(|a| a.norm_sqr()).sum::<f64>() / amplitudes.len() as f64;
    ...
    fn extract_optimal_convexity_from_state(
        &self,
        quantum_state: &QuantumState,
    ) -> QuantumResult<f64> {
        let amplitudes = quantum_state.get_amplitudes();
        let optimal_convexity = amplitudes
            .iter()
            .enumerate()
    ...
    fn extract_barbell_allocation_from_state(
        &self,
        quantum_state: &QuantumState,
    ) -> QuantumResult<(f64, f64)> {
        let amplitudes = quantum_state.get_amplitudes();
        let safe_allocation = amplitudes
            .iter()
            .take(amplitudes.len() / 2)
            .map(|a| a.norm_sqr())
            .sum::<f64>();
        let risky_allocation = amplitudes
            .iter()
            .skip(amplitudes.len() / 2)
            .map(|a| a.norm_sqr())
            .sum::<f64>();
    ...
    fn extract_option_strategy_from_state(
        &self,
        quantum_state: &QuantumState,
        strike_prices: &[f64],
    ) -> QuantumResult<HashMap<String, f64>> {
    ...
    fn calculate_overall_risk_score(
        ...
        let tail_score =
            (tail_risk.var_99.abs() + tail_risk.cvar_99.abs() + tail_risk.max_drawdown.abs()) / 3.0;
        let black_swan_score =
            black_swans.iter().map(|e| e.magnitude).sum::<f64>() / black_swans.len().max(1) as f64;
    ...
    fn generate_recommendations(
        &self,
        tail_risk: &TailRiskMetrics,
        antifragility: &AntifragilityMetrics,
        black_swans: &[BlackSwanEvent],
    ) -> QuantumResult<Vec<String>> {
        ...
        if black_swans.len() > 3 {
            recommendations
                .push("Implement barbell strategy for black swan protection".to_string());
        }
    ...
    fn calculate_upside_capture(&self, stress: &[f64], performance: &[f64]) -> f64 {
        ...
        let corresponding_performance: Vec<f64> = stress
            .iter()
    ...
    fn calculate_asymmetric_payoff_ratio(&self, portfolio: &[f64]) -> f64 {
        let positive_returns = portfolio
            .iter()
            .filter(|&&x| x > 0.0)
            .cloned()
            .collect::<Vec<f64>>();
        let negative_returns = portfolio
            .iter()
            .filter(|&&x| x < 0.0)
            .cloned()
            .collect::<Vec<f64>>();

        let avg_positive = if positive_returns.is_empty() {
            0.0
        } else {
            positive_returns.iter().sum::<f64>() / positive_returns.len() as f64
        };
        let avg_negative = if negative_returns.is_empty() {
            0.0
        } else {
    ...
    fn calculate_tail_hedging_premium(&self, strikes: &[f64], market_data: &[f64]) -> f64 {
        ...
        let current_price = market_data.last().unwrap_or(&100.0);
        let otm_puts = strikes
            .iter()
            .filter(|&&s| s < *current_price * 0.9)
            .count();
    ...
    fn calculate_cvar_from_amplitudes(&self, amplitudes: &[Complex64], confidence: f64) -> f64 {
        ...
        let tail_probs: Vec<f64> = probabilities
            .iter()
            .filter(|&&p| p > var)
            .cloned()
            .collect();
    ...
    pub fn get_metrics(&self) -> Result<QuantumTalebianMetrics, Box<dyn std::error::Error>> {
        self.quantum_metrics.lock()
            .map(|metrics| metrics.clone())
            .map_err(|e| format!("Failed to lock quantum metrics: {}", e).into())
    }
