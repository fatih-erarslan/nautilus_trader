...
pub struct BarbellConfig {
    /// Safe allocation ratio
    pub safe_allocation: f64,
    /// Risky allocation ratio
    pub risky_allocation: f64,
    /// Rebalance threshold
    pub rebalance_threshold: f64,
    /// Maximum risky volatility
    pub max_risky_volatility: f64,
    /// Minimum safe yield
    pub min_safe_yield: f64,
}
...
pub struct StrategyConfig {
    ...
    pub capital_allocation_limit: f64,
    /// Enable stop loss
...
pub struct AllocationResult {
    ...
    /// Safe weight for compatibility
    pub safe_weight: f64,
    /// Risky weight for compatibility
...
impl BarbellStrategy {
    ...
    pub fn calculate_allocation(
        ...
        let result = AllocationResult {
            ...
            timestamp: Utc::now(),
            safe_weight: safe_alloc,
            risky_weight: risky_alloc,
            risk_level: expected_volatility,
        };
    ...
    pub fn check_rebalancing_signals(
        &self,
        market_conditions: &MarketConditions,
    ) -> Result<Vec<RebalancingSignal>, Box<dyn std::error::Error>> {
    ...
    ) -> Result<RebalancingResult, Box<dyn std::error::Error>> {
    ...
    pub fn get_portfolio_metrics(&self) -> Result<PortfolioMetrics, Box<dyn std::error::Error>> {
        ...
        Ok(PortfolioMetrics {
            ...
            risk_budget_utilization: self
                .calculate_risk_budget_utilization(&self.current_allocation)?,
    ...
    fn calculate_dynamic_allocation(
        &self,
        market_conditions: &MarketConditions,
    ...
    fn allocate_risky_assets(
        &self,
        allocations: &mut HashMap<String, f64>,
        total_risky_alloc: f64,
    ...
    fn calculate_expected_volatility(
        &self,
        allocations: &HashMap<String, f64>,
    ) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    fn calculate_risk_budget_utilization(
        &self,
        allocations: &HashMap<String, f64>,
    ) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    /// Check if rebalancing is required
    fn is_rebalancing_required(
        &self,
        new_allocations: &HashMap<String, f64>,
    ...
    /// Check allocation drift
    fn check_allocation_drift(
        &self,
    ) -> Result<Option<RebalancingSignal>, Box<dyn std::error::Error>> {
    ...
    /// Check scheduled rebalancing
    fn check_scheduled_rebalancing(
        &self,
    ) -> Result<Option<RebalancingSignal>, Box<dyn std::error::Error>> {
        if let Some(last_rebalance) = self.last_rebalance {
            ...
            if days_since_rebalance >= self.config.rebalancing_frequency_days as i64 {
                return Ok(Some(RebalancingSignal {
                    signal_strength: (days_since_rebalance as f64
                        / self.config.rebalancing_frequency_days as f64)
                        .min(1.0),
...
mod tests {
    ...
    fn test_rebalancing_signals() {
        ...
        let signals = strategy
            .check_rebalancing_signals(&market_conditions)
            .unwrap();
    ...
    fn test_risk_budget_utilization() {
        ...
        let utilization = strategy
            .calculate_risk_budget_utilization(&allocations)
            .unwrap();
        assert!(utilization > 0.0);
    }
}
/// Barbell allocation result
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BarbellAllocation {
    pub safe_allocation: f64,
    pub risky_allocation: f64,
    pub safe_assets: Vec<AssetAllocation>,
    pub risky_assets: Vec<AssetAllocation>,
    pub expected_return: f64,
    pub risk_level: f64,
}
...
pub struct BarbellEngine {
    strategy: BarbellStrategy,
}

impl BarbellEngine {
    pub fn new(config: MacchiavelianConfig) -> Self {
        let barbell_config = BarbellConfig {
            safe_allocation: config.barbell_safe_ratio,
            risky_allocation: config.barbell_risky_ratio,
            rebalance_threshold: 0.05,
            max_risky_volatility: 0.5,
            min_safe_yield: 0.0,
        };
        ...
        let params = BarbellParams {
            safe_allocation: barbell_config.safe_allocation,
            risky_allocation: barbell_config.risky_allocation,
            min_safe_allocation: 0.6,
            max_risky_allocation: 0.4,
            rebalancing_threshold: barbell_config.rebalance_threshold,
            enable_dynamic_allocation: true,
            risk_budget: 0.02,
        };
        Self {
            strategy: BarbellStrategy::new("barbell_strategy".to_string(), strategy_config, params)
                .expect("Failed to create barbell strategy"),
        }
    }
    ...
    pub fn allocate(
        ...
        let market_conditions = MarketConditions {
            ...
            liquidity_level: market_data.volume / 1000000.0, // Normalize volume
            correlation_breakdown: market_data.volatility > 0.3,
            sentiment_extreme: market_data.volatility > 0.5,
            risk_off_mode: market_data.volatility > 0.4,
        };
        let allocation = self
            .strategy
            .calculate_allocation(&market_conditions)
            .map_err(|e| TalebianRiskError::CalculationError(e.to_string()))?;
        Ok(BarbellAllocation {
            safe_allocation: allocation.safe_weight,
            risky_allocation: allocation.risky_weight,
            safe_assets: vec![],
            risky_assets: vec![],
