...
pub struct FibonacciAnalyzer {
    ...
    alignment_scorer: crate::alignment::AlignmentScorer,
    extension_calculator: crate::extensions::ExtensionCalculator,
    volatility_analyzer: crate::volatility::VolatilityAnalyzer,
...
impl FibonacciAnalyzer {
    ...
    pub fn new(config: FibonacciConfig) -> Self {
        ...
        let alignment_scorer = AlignmentScorer::new(config.alignment_tolerance, 0.9); // 0.9 weight decay
        let extension_calculator = ExtensionCalculator::new();
        let volatility_analyzer = VolatilityAnalyzer::new(config.atr_period, 2.0); // 2.0 band multiplier
    ...
    pub fn analyze(&self, prices: &[f64], volumes: &[f64]) -> FibonacciResult<AnalyzerResult> {
        ...
        // Calculate extensions based on recent swings
        let extension_prices = if !swing_points.swing_highs.is_empty() && !swing_points.swing_lows.is_empty() {
            ...
            self.extension_calculator.calculate_extensions(low_price, high_price, current_price)
        } else {
            vec![]
        };
        
        let target_price = extension_prices.first().copied().unwrap_or(0.0);
        let projection_range = if !extension_prices.is_empty() {
            extension_prices.last().unwrap() - extension_prices.first().unwrap()
        } else {
            0.0
        };
        
        let extensions = ExtensionLevels {
            levels: extension_prices.into_iter().enumerate().map(|(i, price)| {
                (format!("ext_{}", i), price)
            }).collect(),
            trend: retracements.trend,
            base_price: prices.last().copied().unwrap_or(0.0),
            target_price,
            projection_range,
        };
        ...
        Ok(AnalyzerResult {
            signal,
            confidence,
            computation_time_ns: analysis_time.as_nanos() as u64,
            primary_level: retracements.levels.iter()
                .min_by_key(|(_, &level)| {
                    ((level - prices.last().unwrap_or(&0.0)).abs() * 1000.0) as i64
                })
                .map(|(_, &level)| level)
                .unwrap_or(0.0),
            support_levels: retracements.levels.iter()
                .filter(|(_, &level)| level < *prices.last().unwrap_or(&0.0))
                .map(|(_, &level)| level)
                .collect(),
            resistance_levels: retracements.levels.iter()
                .filter(|(_, &level)| level > *prices.last().unwrap_or(&0.0))
                .map(|(_, &level)| level)
                .collect(),
            volatility_bands: volatility_bands.bands.get("atr_1.0")
                .map(|&(upper, lower)| (lower, upper)),
        ...
        for (level_name, &level_ratio) in &self.config.retracement_levels {
            let level_price = match trend {
                ...
                TrendDirection::Sideways | TrendDirection::Unknown => {
                    // For sideways/unknown trends, use mid-point
                    (high_price + low_price) / 2.0 + (level_ratio - 0.5) * price_range
                }
            };
    ...
    fn generate_signal(
        ...
        _extensions: &ExtensionLevels,
        volatility_bands: &crate::core::VolatilityBands,
        ...
        let signal = if combined_signal > 0.2 {
            Signal::Buy
    ...
    pub fn update_config(&mut self, config: FibonacciConfig) {
        ...
        // Re-create alignment scorer and volatility analyzer with new config
        self.alignment_scorer = AlignmentScorer::new(config.alignment_tolerance, 0.9);
        self.volatility_analyzer = VolatilityAnalyzer::new(config.atr_period, 2.0);
        self.config = config; // Move assignment last to avoid borrow issues
    }
