...
pub fn simd_autocorrelation(data: &[f64], lag: usize) -> f64 {
    ...
    if chunks > 0 && lag < n - 4 {
        for i in 0..chunks {
            let idx = i * 4;
            if idx + lag + 4 <= n {
                let chunk1 = f64x4::new([norm_data[idx], norm_data[idx + 1], norm_data[idx + 2], norm_data[idx + 3]]);
                let chunk2 = f64x4::new([norm_data[idx + lag], norm_data[idx + lag + 1], norm_data[idx + lag + 2], norm_data[idx + lag + 3]]);
...
impl SimdRollingWindow {
    ...
    pub fn min_max(&self) -> (f64, f64) {
        ...
        if chunks > 0 {
            for i in 0..chunks {
                let idx = i * 4;
                let chunk = f64x4::new([self.buffer[idx], self.buffer[idx + 1], self.buffer[idx + 2], self.buffer[idx + 3]]);
                
                // Extract individual values for min/max comparison
                let vals = chunk.to_array();
                for &val in &vals {
                    if val < min {
                        min = val;
                    }
