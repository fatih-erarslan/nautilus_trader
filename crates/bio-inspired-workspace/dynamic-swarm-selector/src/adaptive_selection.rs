...
impl AdaptiveSelector {
    ...
    ) -> anyhow::Result<SwarmAlgorithm> {
        if available_algorithms.is_empty() {
            return Err(anyhow::anyhow!("No suitable algorithm found for regime: {:?}", current_regime));
        }
    ...
    async fn ucb_selection(
        ...
        for &algorithm in available_algorithms {
            ...
            let empty_vec = vec![];
            let rewards = algorithm_rewards.get(&algorithm).unwrap_or(&empty_vec);
    ...
    pub async fn update_performance(
        ...
        // Ensure algorithm has a selection count entry (for statistics consistency)
        // Note: This does NOT increment counts - selection tracking happens in select_algorithm()
        // This just ensures the algorithm appears in stats when only update_performance is called
        {
            let mut counts = self.selection_counts.write().await;
            counts.entry(algorithm).or_insert(0);
        }
