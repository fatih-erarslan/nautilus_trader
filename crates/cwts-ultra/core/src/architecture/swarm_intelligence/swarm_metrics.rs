...
impl SwarmBehaviorMonitor {
    ...
    pub async fn calculate_collective_efficiency(
        &self,
        ...
        let harmonic_mean = weighted_performances.len() as f64
            / weighted_performances
                .iter()
                .map(|x| 1.0 / x.max(1e-10))
                .sum::<f64>();

        // Apply collective intelligence amplification factor
        let amplification_factor =
            (agent_performances.len() as f64).ln() / (agent_performances.len() as f64).sqrt();

        (harmonic_mean * amplification_factor).min(1.0)
    }

    /// Measure information propagation speed across E2B sandboxes
    pub async fn measure_propagation_speed(
        &self,
        propagation_analyzer: &InformationPropagationAnalyzer,
    ) -> Duration {
    ...
    pub async fn assess_pattern_stability(
        &self,
        ...
        for pattern in patterns {
            ...
            let combined_stability =
                temporal_stability * confidence_stability * performance_stability;
    ...
    pub async fn update_metrics(
        &self,
        sandbox_performances: HashMap<String, SandboxPerformance>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        // Create E2B sandbox metrics
        let e2b_metrics = E2BSandboxMetrics {
            training_sandbox_metrics: sandbox_performances
                .get(E2B_BAYESIAN_TRAINING)
                .cloned()
                .unwrap_or_else(|| Self::default_sandbox_performance(E2B_BAYESIAN_TRAINING)),
            validation_sandbox_metrics: sandbox_performances
                .get(E2B_MONTE_CARLO_VALIDATION)
                .cloned()
                .unwrap_or_else(|| Self::default_sandbox_performance(E2B_MONTE_CARLO_VALIDATION)),
            realtime_sandbox_metrics: sandbox_performances
                .get(E2B_REALTIME_PROCESSING)
                .cloned()
                .unwrap_or_else(|| Self::default_sandbox_performance(E2B_REALTIME_PROCESSING)),
            cross_sandbox_coherence: self
                .calculate_cross_sandbox_coherence(&sandbox_performances)
                .await,
    ...
    async fn calculate_cross_sandbox_coherence(
        &self,
        performances: &HashMap<String, SandboxPerformance>,
    ) -> f64 {
        let sandbox_accuracies: Vec<f64> = performances
            .values()
        ...
        let variance = sandbox_accuracies
            .iter()
            .map(|&acc| (acc - mean).powi(2))
            .sum::<f64>()
            / sandbox_accuracies.len() as f64;
    ...
    pub async fn generate_intelligence_report(&self) -> String {
        ...
        if history.len() > 1 {
            ...
            let efficiency_trend = Self::calculate_trend(
                &history
                    .iter()
                    .map(|m| m.collective_var_estimation_efficiency)
                    .collect::<Vec<_>>(),
            );
            report.push_str(&format!(
                "- **Efficiency Trend**: {}\n",
        ...
        report.push_str("\n---\n");
        report.push_str(
            "*Report generated by Swarm Intelligence Monitoring System with E2B Integration*\n",
        );
    ...
    fn default_metrics() -> SwarmIntelligenceMetrics {
        SwarmIntelligenceMetrics {
            ...
            e2b_sandbox_performance: E2BSandboxMetrics {
                ...
                validation_sandbox_metrics: Self::default_sandbox_performance(
                    E2B_MONTE_CARLO_VALIDATION,
                ),
                realtime_sandbox_metrics: Self::default_sandbox_performance(
...
mod tests {
    ...
    async fn test_swarm_monitor_initialization() {
        let monitor = SwarmBehaviorMonitor::new(Duration::from_secs(1), 100);
    ...
    async fn test_metrics_update() {
        ...
        sandbox_performances.insert(
            E2B_BAYESIAN_TRAINING.to_string(),
            SandboxPerformance {
