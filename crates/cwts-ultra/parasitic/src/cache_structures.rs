...
impl NumaAllocator {
    ...
    pub unsafe fn alloc_aligned<T>(&self, count: usize) -> *mut T {
        let layout =
            Layout::from_size_align(size_of::<T>() * count, CACHE_LINE_SIZE.max(align_of::<T>()))
                .unwrap();
...
impl<T> FastMemoryPool<T> {
    ...
    pub fn deallocate(&self, ptr: *mut T) {
        ...
        let node_ptr = unsafe {
            let data_offset =
                ptr as usize - &(*(ptr::null::<PoolNode<T>>())).data as *const T as usize;
    ...
    pub fn utilization(&self) -> f32 {
        let allocated = self.allocated_count.load(Ordering::Acquire);
    ...
    pub fn prefetch_pool(&self) {
        unsafe {
            self.numa_allocator
                .prefetch_read(self.pool_memory, self.total_capacity * self.node_size);
        }
    }
}
...
impl<T: Copy> CacheOptimizedRingBuffer<T> {
    ...
    pub fn new(capacity_log2: u8, num_consumers: usize, numa_node: u32) -> Self {
        ...
        let layout = Layout::from_size_align(size_of::<T>() * capacity, CACHE_LINE_SIZE).unwrap();
    ...
    pub fn pop(&self, consumer_id: usize) -> Option<T> {
        ...
        unsafe {
            self.numa_allocator
                .prefetch_read(self.buffer.add(next_index), 4);
        }
    ...
    pub fn pop_batch(&self, consumer_id: usize, batch: &mut [T]) -> usize {
        ...
        unsafe {
            self.numa_allocator
                .prefetch_read(self.buffer.add(start_index), to_read);
        }
...
impl<K: Copy + Eq, V: Copy> FastHashTable<K, V> {
    ...
    pub fn get(&self, key: &K) -> Option<V> {
        ...
        unsafe {
            self.numa_allocator
                .prefetch_read(self.buckets.add(index), 4);
        }
    ...
    pub fn load_factor(&self) -> f32 {
        self.size.load(Ordering::Acquire) as f32 / self.capacity as f32
    }
}
...
mod tests {
    ...
    fn test_numa_allocator() {
        ...
        unsafe {
            allocator.prefetch_read(ptr, 10);
        }
    }
