...
def refine_agent(agent: SophisticatedQLearningAgent, environment: EnvironmentWrapper,
    ...
    while not converged and refinement_count < max_refinements:
        ...
        new_states = agent.states + max(10, int(agent.states * 0.2))  # Add 20% more states
        new_actions = agent.actions  # Keep actions constant for most environments
        agent.resize_q_table(new_states, new_actions)

        # Increase training parameters
        agent.max_episodes += 5000  # More training episodes
        agent.max_steps_per_episode += 50  # More steps per episode

        # Train the agent
        converged, episodes = agent.train(environment)

        # Record refinement point for plotting
        agent.metrics.add_refinement_point(
            sum(len(agent.metrics.episode_rewards) for _ in range(refinement_count))
        )

        # Evaluate the agent
        evaluation_results = agent.evaluate(environment, num_episodes=evaluation_episodes)
        current_performance = evaluation_results['mean_reward']

        # Check if this is the best performing agent so far
        if current_performance > best_performance:
            best_performance = current_performance
            # Create a copy of the current agent
            agent_path = f"temp_agent_{refinement_count}.pkl"
            agent.save(agent_path)
            best_agent = agent_path

        refinement_count += 1

        logger.info(f"Refinement {refinement_count}/{max_refinements} completed")
        logger.info(f"  States: {agent.states}, Actions: {agent.actions}")
        logger.info(f"  Performance: {current_performance:.2f}, Best: {best_performance:.2f}")
        logger.info(f"  Converged: {converged}, Episodes: {episodes}")

    # Load the best agent
    if best_agent is not None:
        agent = SophisticatedQLearningAgent.load(best_agent)

        # Clean up temporary files
        for i in range(refinement_count):
            temp_path = f"temp_agent_{i}.pkl"
...
def run_demonstration():
    """Run a demonstration of the Q* Learning algorithm."""
    logger.info("Starting Q* Learning demonstration")

    # Create environment
    if GYM_AVAILABLE:
        try:
            env = EnvironmentWrapper(gym_env_name="FrozenLake-v1")
            logger.info("Using Gym FrozenLake-v1 environment")
        except Exception as e:
            logger.warning(f"Error creating Gym environment: {e}")
            env = EnvironmentWrapper()
            logger.info("Using default random environment")
    else:
        env = EnvironmentWrapper()
        logger.info("Using default random environment")
    ...
    # Train the agent
    converged, episodes = agent.train(env)

    # Plot metrics
    agent.metrics.plot_metrics()

    # Evaluate the agent
    agent.evaluate(env, num_episodes=100, render=False)

    # Refine the agent
    logger.info("Refining agent...")
    refined_agent = refine_agent(agent, env, max_refinements=3)

    # Evaluate refined agent
    refined_agent.evaluate(env, num_episodes=100, render=False)

    # Plot refined agent metrics
    refined_agent.metrics.plot_metrics("refined_agent_metrics.png")

    # Save refined agent
    refined_agent.save("refined_q_star_agent.pkl")

    logger.info("Demonstration completed")


if __name__ == "__main__":
    # Run demonstration with full features
    run_demonstration()