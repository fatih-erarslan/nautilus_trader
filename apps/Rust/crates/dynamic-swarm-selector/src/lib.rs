...
pub trait SwarmOptimizer: Send + Sync {
    async fn optimize(&mut self, 
                     ...
                     parameters: &OptimizationParameters) -> Result<OptimizationResult>;
    
    async fn update_population(&mut self, performance_feedback: &PerformanceFeedback) -> Result<()>;
...
pub trait ObjectiveFunction: Send + Sync {
    async fn evaluate(&self, solution: &Solution) -> Result<f64>;
