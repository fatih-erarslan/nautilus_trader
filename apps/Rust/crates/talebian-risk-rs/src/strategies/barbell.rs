...
use crate::error::{TalebianError, TalebianResult as Result};
...
impl BarbellStrategy {
    ...
    fn calculate_market_stress(&self, market_data: &MarketData) -> Result<f64> {
        ...
        // Volatility stress
        let avg_volatility =
            market_data.volatilities.values().sum::<f64>() / market_data.volatilities.len() as f64;
        ...
        let avg_correlation =
            market_data.correlations.values().sum::<f64>() / market_data.correlations.len() as f64;
    ...
    pub fn get_barbell_metrics(&self) -> BarbellMetrics {
        let safe_weight = self
            .portfolio
            .weights
            .iter()
            .filter(|(asset, _)| {
                self.portfolio.asset_types.get(*asset).map_or(false, |&t| {
                    matches!(t, AssetType::Safe | AssetType::Moderate)
                })
            })
            .map(|(_, weight)| weight)
            .sum::<f64>();
        ...
        BarbellMetrics {
            ...
            barbell_ratio: if risky_weight > 0.0 {
                safe_weight / risky_weight
            } else {
                f64::INFINITY
            },
...
impl TalebianStrategy for BarbellStrategy {
    ...
    fn update_strategy(&mut self, market_data: &MarketData) -> Result<()> {
        ...
        if self.needs_rebalancing(&new_weights) {
            ...
            self.portfolio.concentration_metrics =
                ConcentrationMetrics::calculate(&self.portfolio.weights);
    ...
    fn robustness_assessment(&self, scenarios: &[MarketScenario]) -> Result<RobustnessAssessment> {
        ...
        let best_case = performances
            .iter()
            .fold(f64::NEG_INFINITY, |a, &b| a.max(b));

        // Calculate robustness score
        let robustness_score = if worst_case < -0.5 {
            0.0
        } else {
            1.0 - worst_case.abs()
        };
...
impl BarbellStrategy {
    ...
    fn metadata(&self) -> HashMap<String, String> {
        ...
        metadata.insert(
            "safe_allocation".to_string(),
            self.safe_allocation.to_string(),
        );
        metadata.insert(
            "risky_allocation".to_string(),
            self.risky_allocation.to_string(),
        );
        metadata.insert(
            "num_positions".to_string(),
            self.portfolio.num_positions.to_string(),
        );
        metadata.insert(
            "performance_records".to_string(),
            self.performance_history.len().to_string(),
        );
        metadata
    }
}
...
mod tests {
    ...
    fn test_barbell_metrics() {
        ...
        strategy
            .portfolio
            .asset_types
            .insert("BONDS".to_string(), AssetType::Safe);
        strategy
            .portfolio
            .asset_types
            .insert("STOCKS".to_string(), AssetType::Volatile);
    ...
    fn test_position_size_calculation() {
        ...
        let positions = strategy
            .calculate_position_sizes(&assets, &market_data)
            .unwrap();
