...
use crate::{PerformanceTracker, ReturnData};
...
impl TalebianRisk {
    ...
    pub fn update_market_data(
        ...
        let current_value = if self.portfolio_value_history.is_empty() {
            ...
            let last_value = self.portfolio_value_history.last().unwrap_or(&100000.0);
        ...
        self.last_update = Some(
            chrono::DateTime::from_timestamp(return_data.timestamp_unix, 0)
    ...
    fn compute_detailed_metrics(&self) -> Result<RiskMetrics, Box<dyn std::error::Error>> {
        ...
        let tail_returns: Vec<f64> = sorted_returns.iter().take(var_index + 1).copied().collect();
    ...
    fn calculate_overall_risk_level(&self, metrics: &RiskMetrics) -> f64 {
        ...
        // Tail risk
        let tail_risk = if metrics.tail_ratio < 1.0 {
            1.0 - metrics.tail_ratio
        } else {
            0.0
        };
    ...
    fn generate_recommendations(
        ...
        if metrics.current_drawdown > self.config.max_drawdown * 0.8 {
            recommendations
                .push("Consider reducing position sizes to limit further drawdown".to_string());
        }

        if metrics.volatility > self.config.target_volatility * 1.3 {
            recommendations
                .push("Implement volatility targeting to reduce portfolio risk".to_string());
        }

        if metrics.sharpe_ratio < 0.5 {
            recommendations
                .push("Review strategy performance and consider diversification".to_string());
        }

        if metrics.black_swan_probability > 0.005 {
            recommendations
                .push("Increase tail risk hedging and consider barbell strategy".to_string());
        }

        if !warnings.is_empty() {
            recommendations
                .push("Monitor positions closely and prepare for risk reduction".to_string());
        }
    ...
    fn calculate_antifragility_score(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let vol_performance =
            performance_in_vol.iter().sum::<f64>() / performance_in_vol.len().max(1) as f64;

        let antifragility = if baseline_performance != 0.0 {
            ((vol_performance - baseline_performance) / baseline_performance.abs())
                .max(-1.0)
                .min(1.0)
        } else {
            0.0
...
mod tests {
    ...
    fn test_risk_category_classification() {
        ...
        assert!(matches!(
            risk_system.classify_risk_level(0.1),
            RiskCategory::Low
        ));
        assert!(matches!(
            risk_system.classify_risk_level(0.3),
            RiskCategory::Moderate
        ));
        assert!(matches!(
            risk_system.classify_risk_level(0.6),
            RiskCategory::High
        ));
        assert!(matches!(
            risk_system.classify_risk_level(0.9),
            RiskCategory::Extreme
        ));
        assert!(matches!(
            risk_system.classify_risk_level(0.99),
            RiskCategory::BlackSwan
        ));
    }
