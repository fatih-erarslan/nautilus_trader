...
use crate::core::{AssetType, RiskConfig, TalebianRiskComponent};
...
impl AntifragilePortfolio {
    ...
    pub fn remove_asset(&mut self, asset: &str) -> Result<()> {
        if self.positions.remove(asset).is_none() {
            return Err(TalebianError::portfolio_construction(format!(
    ...
    pub fn update_weight(&mut self, asset: &str, new_weight: f64) -> Result<()> {
        ...
        if let Some(position) = self.positions.get_mut(asset) {
            ...
            Err(TalebianError::portfolio_construction(format!(
                "Asset {} not found in portfolio",
                asset
            )))
        }
    ...
    pub fn calculate_total_value(&mut self) -> f64 {
        self.total_value = self.positions.values().map(|pos| pos.value).sum();
        self.total_value
    }
    ...
    pub fn calculate_risk_metrics(&self) -> Result<PortfolioRiskMetrics> {
        ...
        for position in self.positions.values() {
            match position.asset_type {
                ...
                AssetType::Volatile | AssetType::Derivative | AssetType::Risky => {
                    risky_allocation += position.weight
                }
                AssetType::Antifragile | AssetType::Alternative => {
                    antifragile_allocation += position.weight
                }
    ...
    pub fn validate_constraints(&self) -> Result<()> {
        ...
        if (total_weight - 1.0).abs() > 0.01 {
            return Err(TalebianError::portfolio_construction(format!(
                "Portfolio weights sum to {:.3}, should be 1.0",
                total_weight
            )));
        }
        ...
        for (asset, position) in &self.positions {
            if position.weight > self.config.max_position_size {
                return Err(TalebianError::portfolio_construction(format!(
                    "Asset {} weight {:.3} exceeds maximum {:.3}",
            ...
            if let Some(position) = self.positions.get(asset) {
                ...
                if weight_diff.abs() > 0.001 {
                    ...
                    actions.push(RebalanceAction {
                        asset: asset.clone(),
                        action_type: if weight_diff > 0.0 {
                            ActionType::Buy
                        } else {
                            ActionType::Sell
                        },
...
mod tests {
    ...
    fn test_update_weight() {
        ...
        portfolio
            .add_asset("BTC", 0.1, AssetType::Volatile)
            .unwrap();
    ...
    fn test_antifragility_measurement() {
        ...
        portfolio
            .add_asset("GOLD", 0.3, AssetType::Antifragile)
            .unwrap();
        portfolio
            .add_asset("OPTIONS", 0.2, AssetType::Derivative)
            .unwrap();
    ...
    fn test_risk_metrics() {
        ...
        portfolio
            .add_asset("STOCKS", 0.3, AssetType::Volatile)
            .unwrap();
        portfolio
            .add_asset("GOLD", 0.1, AssetType::Antifragile)
            .unwrap();
    ...
    fn test_portfolio_validation() {
        ...
        portfolio
            .add_asset("STOCKS", 0.5, AssetType::Volatile)
            .unwrap();
        portfolio.add_asset("BONDS", 0.5, AssetType::Safe).unwrap();
    ...
    fn test_rebalancing() {
        ...
        portfolio
            .add_asset("STOCKS", 0.6, AssetType::Volatile)
            .unwrap();
