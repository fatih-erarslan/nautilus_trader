...
pub enum RegimeChange {
    VolatilityIncrease { from: f64, to: f64 },
    VolatilityDecrease { from: f64, to: f64 },
    TrendChange { from: TrendDirection, to: TrendDirection },
    MarketCrash { severity: f64 },
    Recovery { strength: f64 },
}
...
impl AntifragilityMeasurer {
    ...
    pub fn update(&mut self, returns: f64, volatility: f64, timestamp: DateTime<Utc>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn measure_antifragility(&mut self) -> Result<AntifragilityMeasurement, Box<dyn std::error::Error>> {
        ...
        let scores = [convexity, volatility_benefit, stress_response, hormesis_effect, tail_benefit, regime_adaptation];
    ...
    fn is_stress_event(&self, current_volatility: f64) -> bool {
        ...
        let recent_avg_vol = self.volatility_history.iter()
            .rev()
            .take(20)
            .sum::<f64>() / 20.0;
    ...
    fn record_stress_event(&mut self, timestamp: DateTime<Utc>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pre_stress_performance = pre_stress_returns.iter().sum::<f64>() / pre_stress_returns.len() as f64;
        let during_stress_performance = stress_returns.iter().sum::<f64>() / stress_returns.len() as f64;
        let post_stress_performance = post_stress_returns.iter().sum::<f64>() / post_stress_returns.len() as f64;
    ...
    fn detect_regime_change(&self) -> Option<RegimeChange> {
        ...
        let current_vol = self.volatility_history.iter()
            .rev()
            .take(current_window)
            .sum::<f64>() / current_window as f64;

        let previous_vol = self.volatility_history.iter()
            .rev()
            .skip(current_window)
            .take(previous_window)
            .sum::<f64>() / previous_window as f64;
    ...
    fn record_adaptation(&mut self, regime_change: RegimeChange, timestamp: DateTime<Utc>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn calculate_convexity(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let covariance = recent_returns.iter().zip(recent_volatility.iter())
            .map(|(r, v)| (r - mean_returns) * (v - mean_vol))
            .sum::<f64>() / (recent_returns.len() - 1) as f64;

        let vol_variance = recent_volatility.iter()
            .map(|v| (v - mean_vol).powi(2))
            .sum::<f64>() / (recent_volatility.len() - 1) as f64;

        let returns_variance = recent_returns.iter()
    ...
    fn calculate_volatility_benefit(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let volatility_threshold = self.volatility_history.iter().sum::<f64>() / self.volatility_history.len() as f64;
    ...
    fn calculate_hormesis_effect(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let small_stress_events: Vec<_> = self.stress_events.iter()
        ...
        let hormesis_score = small_stress_events.iter()
            .map(|event| {
                let improvement = event.post_stress_performance - event.pre_stress_performance;
                improvement.max(0.0)
            })
            .sum::<f64>() / small_stress_events.len() as f64;
    ...
    fn calculate_regime_adaptation(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let avg_adaptation_quality = self.adaptation_history.iter()
