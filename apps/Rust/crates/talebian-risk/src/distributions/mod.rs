...
impl ExtremeValueStats {
    ...
    pub fn calculate_return_levels(&mut self, periods: &[f64], distribution: &dyn FatTailDistribution) -> Result<()> {
...
pub mod utils {
    ...
    pub fn kolmogorov_smirnov_test(data: &[f64], distribution: &dyn FatTailDistribution) -> Result<(f64, f64)> {
        if data.is_empty() {
            return Err(TalebianError::insufficient_data(1, 0));
        }
