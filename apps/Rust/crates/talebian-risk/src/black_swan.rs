...
pub struct BlackSwanDetector {
    ...
    detected_events: Vec<BlackSwanEvent>,
    warning_signals: Vec<WarningSignal>,
    baseline_statistics: Option<BaselineStatistics>,
}
...
impl BlackSwanDetector {
    ...
    pub fn update(&self, _returns: f64, _timestamp: DateTime<Utc>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn detect_black_swan(&mut self, returns: f64, timestamp: DateTime<Utc>) -> Result<Option<BlackSwanEvent>, Box<dyn std::error::Error>> {
        // Add new data
                ...
                if impact > self.params.min_impact_magnitude {
                    ...
                    if probability < self.params.rarity_threshold {
                        let event = self.create_black_swan_event(
                            returns, timestamp, magnitude, impact, probability
                        )?;
    ...
    pub fn calculate_black_swan_probability(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let extreme_events = self.return_history.iter()
            .filter(|&&r| {
                ...
                z_score.abs() > self.params.extreme_threshold && r.abs() > self.params.min_impact_magnitude
            })
            .count();
    ...
    pub fn analyze_event_clustering(&self) -> Result<ClusteringAnalysis, Box<dyn std::error::Error>> {
        ...
        let cluster_tendency = if cluster_count == 0 { 0.0 } else { 
    ...
    pub fn get_summary(&self) -> BlackSwanSummary {
        ...
        let average_severity = if total_events > 0 {
            self.detected_events.iter().map(|e| e.magnitude).sum::<f64>() / total_events as f64
        } else {
            0.0
        };
        let max_severity = self.detected_events.iter()
            .map(|e| e.magnitude)
            .fold(0.0, f64::max);
        let total_warnings = self.warning_signals.len();
        let probability_estimate = self.calculate_black_swan_probability().unwrap_or(0.0);
        let clustering_tendency = self.analyze_event_clustering()
    ...
    fn calculate_event_probability(&self, magnitude: f64) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    fn analyze_predictability(&self, event_timestamp: DateTime<Utc>) -> Result<PredictabilityAnalysis, Box<dyn std::error::Error>> {
        ...
        let recent_warnings: Vec<WarningSignal> = self.warning_signals.iter()
        ...
        let predictability_score = if recent_warnings.is_empty() {
            ...
            let max_strength = recent_warnings.iter()
                .map(|s| s.strength)
                .fold(0.0, f64::max);
            max_strength * 0.5 // Scale down as Black Swans are inherently unpredictable
        };

        let warning_time = recent_warnings.first()
    ...
    fn detect_warning_signals(&mut self, _returns: f64, _timestamp: DateTime<Utc>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn calculate_recent_volatility(&self, window: usize) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    fn calculate_probability_adjustment(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let baseline_vol = self.baseline_statistics.as_ref()
            .map(|s| s.std_dev)
            .unwrap_or(0.2);
...
mod tests {
    ...
    fn test_black_swan_detection() {
        ...
        for i in 0..100 {
            ...
            detector.detect_black_swan(normal_return, Utc::now()).unwrap();
        }
        
        // Add extreme return (should trigger Black Swan detection)
        let extreme_return = -0.15; // 15% loss
        let result = detector.detect_black_swan(extreme_return, Utc::now()).unwrap();
    ...
    fn test_warning_signal_detection() {
        ...
        for i in 0..10 {
            detector.return_history.push(0.05 * ((i % 2) as f64 * 2.0 - 1.0)); // Alternating +5% / -5%
        }
