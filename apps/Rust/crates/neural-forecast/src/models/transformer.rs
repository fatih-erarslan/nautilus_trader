...
/// Transformer model for sequence-to-sequence forecasting
#[derive(Debug)]
...
impl TransformerModel {
    ...
    pub fn new_from_config(config: TransformerConfig) -> Result<Self> {
        Ok(Self {
            config,
...
impl Model for TransformerModel {
    ...
    fn new(config: Self::Config) -> Result<Self> {
        match config {
            super::DynamicModelConfig::Transformer(transformer_config) => {
                transformer_config.validate()?;
                Self::new_from_config(transformer_config)
            }
            _ => Err(NeuralForecastError::ConfigError(
                "Invalid config type for Transformer model".to_string()
            )),
        }
    }
    ...
    fn metadata(&self) -> ModelMetadata {
        ModelMetadata {
            ...
            description: "Transformer for multi-asset correlation".to_string(),
            author: "TENGRI Trading Swarm".to_string(),
            created_at: chrono::Utc::now(),
            modified_at: chrono::Utc::now(),
            size_bytes: 0,
            num_parameters: 0,
            input_shape: vec![self.config.input_length, self.config.d_model],
            output_shape: vec![self.config.output_length, self.config.d_model],
