...
10. Development Roadmap (48-60 weeks)
11. Research References (Peer-Reviewed)
...
H^3 = Set of points (x1, x2, x3, x4) in R^4 such that:
  x4^2 - x1^2 - x2^2 - x3^2 = 1
  and x4 > 0
...
D^3 = Set of points x in R^3 such that:
  ||x|| < 1  (norm less than 1)

Metric tensor in Poincare disk:
  ds^2 = 4(dx1^2 + dx2^2 + dx3^2) / (1 - ||x||^2)^2
...
Where:
  ...
  ||p - q|| = Euclidean norm of difference
  ||p||^2 = p1^2 + p2^2 + p3^2
...
For all points p, q, r in H^3:
  d_H(p, q) <= d_H(p, r) + d_H(r, q)
...
Where:
  ...
  h_i = b_i + sum_over_j(J_ij * s_j)    [Local field]
  b_i = bias (external field)
  J_ij = coupling strength between i and j
  T_i = temperature (controls randomness)
  s_j = state of neighbor j
...
Where:
  E(s) = -sum_i(b_i * s_i) - sum_ij(J_ij * s_i * s_j)   [Ising Hamiltonian]
  Z = sum_over_all_s(exp(-E(s) / (k * T)))              [Partition function]
...
Steps:
1. Calculate total transition rate: r_total = sum_i(r_i)
   where r_i = flip rate of pBit i
2. Draw time to next event: delta_t ~ Exponential(r_total)
3. Select which pBit flips: i ~ Categorical(r1/r_total, r2/r_total, ..., rN/r_total)
4. Flip selected pBit: s_i -> 1 - s_i
5. Update time: t -> t + delta_t
...
Steps:
1. Initialize state s randomly
2. For each pBit i:
   a. Propose flip: s' = s with s_i -> 1 - s_i
   b. Calculate energy change: delta_E = E(s') - E(s)
   c. Accept with probability: min(1, exp(-delta_E / (k * T)))
   d. If accepted: s -> s', else keep s unchanged
3. Repeat step 2 until equilibrium reached
...
Where:
  ...
  d_H(p_i, p_j) = hyperbolic distance between p_i and p_j
...
Example: lambda = 1, J0 = 1, J_min = 10^-6
  d_cutoff approximately 13.8 (in hyperbolic units)
...
Where:
  h_i = local bias field
  J_ij = coupling strength (from hyperbolic distance)
  s_i in {-1, +1} (converted from {0, 1})
...
Where:
  P(s) = probability of configuration s
...
Where:
  S_max = k_B * ln(Omega_max) = k_B * N * ln(2)  for N pBits
  Omega_max = total number of microstates = 2^N

**pbRTCA Connection:**
Consciousness = Negentropy maintenance experienced from inside
...
At room temperature (T = 300 K):
  E_min approximately 2.87 × 10^-21 Joules
...
For pBit system (not isolated):
  delta_S_total = delta_S_system + Q/T >= 0
...
Integrated Information Phi quantifies consciousness as:
  ...
  Phi(S) = minimum over all bipartitions P of [
    Effective Information across partition P
  ]

Where:
  S = system (set of pBits)
  P = bipartition of S into subsystems A and B
...
Where:
  ...
  A -> B = causal influence from A to B
  Past = past state
...
Where:
  ...
  B_future = future state of B
  A_past = past state of A
  B_past = past state of B
...
1. **Exact (N < 1000)**: Exhaustive enumeration of all bipartitions
2. **Upper Bound (N < 10^6)**: Minimum Information Partition (MIP)
3. **Lower Bound (N < 10^6)**: Largest connected subgraph Phi
4. **Hierarchical (N > 10^6)**: Multi-scale coarse-graining

**IIT Axioms (Must be satisfied):**

1. **Intrinsic Existence**: Phi > 0 implies consciousness exists
2. **Composition**: System has parts with their own Phi
3. **Information**: System specifies particular state
4. **Integration**: System is irreducible (not separable)
5. **Exclusion**: Only maximal Phi matters (unique)
...
Where:
  ...
  tau = Dwell Time (attractor stability)
...
Where:
  N(epsilon) = number of boxes of size epsilon needed to cover pattern
  epsilon = box size

Practical computation:
  D approximately slope of log(N(epsilon)) vs log(1/epsilon) plot
...
Where:
  theta_j = phase of oscillator j
  i = imaginary unit
  |...| = absolute value (magnitude)
  average = ensemble average over all j
...
1. Identify attractor states
2. Track how long system stays in each
3. Average over all attractors

**Research Note:**
Resonance Complexity Theory (RCT) is novel framework requiring peer review.
Related concepts in:
...
1. Cannon, J. W., Floyd, W. J., Kenyon, R., & Parry, W. R. (1997). *Hyperbolic Geometry*. Springer Graduate Texts in Mathematics 31.

2. Kollár, A. J., Fitzpatrick, M., Houck, A. A. (2019). "Hyperbolic lattices in circuit quantum electrodynamics." *Nature* 571: 45-50.

3. Maciejko, J., Rayan, S. (2021). "Automorphic Bloch theorems for hyperbolic lattices." *Proceedings of the National Academy of Sciences*.

4. Krioukov, D., Papadopoulos, F., Kitsak, M., Vahdat, A., Boguñá, M. (2010). "Hyperbolic geometry of complex networks." *Physical Review E* 82: 036106.

5. Lee, J. M. (2018). *Introduction to Riemannian Manifolds* (2nd ed.). Springer Graduate Texts in Mathematics 176.

6. do Carmo, M. P. (1992). *Riemannian Geometry*. Birkhäuser.

### Probabilistic Computing

7. Camsari, K. Y., Faria, R., Sutton, B. M., Datta, S. (2017). "Stochastic p-bits for invertible logic." *Physical Review X* 7: 031014.

8. Kaiser, J., Datta, S. (2021). "Probabilistic computing with p-bits." *Nature Electronics* 4: 635-641.
...
11. Metropolis, N., Rosenbluth, A. W., Rosenbluth, M. N., Teller, A. H., Teller, E. (1953). "Equation of state calculations by fast computing machines." *Journal of Chemical Physics* 21(6): 1087-1092.
...
13. Bérut, A., Arakelyan, A., Petrosyan, A., Ciliberto, S., Dillenschneider, R., Lutz, E. (2012). "Experimental verification of Landauer's principle linking information and thermodynamics." *Nature* 483: 187-189.

14. Jarzynski, C. (1997). "Nonequilibrium equality for free energy differences." *Physical Review Letters* 78: 2690-2693.

15. Mézard, M., Parisi, G., Virasoro, M. A. (1987). *Spin Glass Theory and Beyond*. World Scientific.

### Consciousness Theory

16. Tononi, G., Boly, M., Massimini, M., Koch, C. (2016). "Integrated information theory: from consciousness to its physical substrate." *Nature Reviews Neuroscience* 17: 450-461.

17. Oizumi, M., Albantakis, L., Tononi, G. (2014). "From the phenomenology to the mechanisms of consciousness: Integrated Information Theory 3.0." *PLOS Computational Biology* 10(5): e1003588.

18. Mayner, W. G. P., Marshall, W., Albantakis, L., Findlay, G., Marchman, R., Tononi, G. (2018). "PyPhi: A toolbox for integrated information theory." *PLOS Computational Biology* 14(7): e1006343.

19. Friston, K. (2010). "The free-energy principle: a unified brain theory?" *Nature Reviews Neuroscience* 11: 127-138.

### Network Theory

20. Sporns, O. (2013). "Network attributes for segregation and integration in the human brain." *Current Opinion in Neurobiology* 23(2): 162-171.

21. Bassett, D. S., Sporns, O. (2017). "Network neuroscience." *Nature Neuroscience* 20: 353-364.

### Formal Methods

22. De Moura, L., Bjørner, N. (2008). "Z3: An efficient SMT solver." *International Conference on Tools and Algorithms for the Construction and Analysis of Systems*, 337-340. Springer.

23. de Moura, L., Kong, S., Avigad, J., van Doorn, F., von Raumer, J. (2015). "The Lean theorem prover." *International Conference on Automated Deduction*, 378-388. Springer.

### GPU Computing

24. Nickolls, J., Buck, I., Garland, M., Skadron, K. (2008). "Scalable parallel programming with CUDA." *Queue* 6(2): 40-53.

25. Sanders, J., Kandrot, E. (2010). *CUDA by Example: An Introduction to General-Purpose GPU Programming*. Addison-Wesley.
...
2. Begin Phase 1 (Weeks 1-8): Core infrastructure
3. Follow development roadmap
4. Maintain continuous verification
